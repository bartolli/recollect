...
def mock_trace_store() -> AsyncMock:
    ...
    s.archive_trace = AsyncMock(return_value=True)
    s.forget_trace = AsyncMock(return_value=True)
    s.reactivate_trace = AsyncMock(return_value=True)
    s.apply_strength_factor = AsyncMock()
    s.apply_decay_factor = AsyncMock()
...
def mock_fact_store() -> AsyncMock:
    ...
    f.get_facts_by_source_trace_id = AsyncMock(return_value=[])
    ...
    f.update_fact_status = AsyncMock(return_value=True)
